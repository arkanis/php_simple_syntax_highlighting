...
$languages = [
		...
		(
			(?:
			    (?:
			        ...
			        | (?<keyword> <\? (?: php | = )? ) (?<lang_php> [\s\S]*? (?: (?: " [^"]*? " | ' [^']*? ' | <<< '? (?<_id> \w+ ) '? \n [\s\S]+? \g{_id} ) [\s\S]*? )* ) (?= \?> )
			    ) | (?<keyword>
			        < !? \w+ (?<lang_html_attr> [^>]+ )? >
			        | </ \w+ >
			        | \?>
	...
	"js" => <<<'EOD'
		(
			(?: (?<keyword> \b (?: import | as | from | export | function | async | class | extends | static | implements | interface | package | private | protected | public | get | set | this | super | new | delete | instanceof | typeof | enum | void ) \b )
			...
			|   (?<value>   \b (?: true | false | null | undefined | arguments
			                |   (?i: 0x [0-9a-f_]+ | 0b [01_] | 0o? [0-7_]+ ) n?
			                |   [-+]? (?: \d [\d_]* (?: \. [\d_]+ )? | \. [\d_]+ ) (?: [eE] [-+]? [\d_]+ )? n?
			                ) )
			|   (?<comment> //.* | /\* [\s\S]+? \*/ )
			|   (?<string>  (?: " [\s\S]*? (?<!\\) " | ' [\s\S]*? (?<!\\) ' | ` [\s\S]*? (?<!\\) ` ) | / [\s\S]+? (?<!\\) / [gmiyuvsd]* )
			|   (?<call>    \b \w+ (?= \( ) )
			)
		)x
	EOD,
	...
	"php" => <<<'EOD'
		(
			(?: (?<keyword> => | & | \b (?<! \$ ) (?: function | use | fn | namespace | class | trait | interface | extends | implements | abstract | final | private | protected | public | readonly | and | or | xor | clone | new | unset | include | include_once | require | require_once | instanceof | insteadof | list | self | static | parent | __CLASS__ | __DIR__ | __FILE__ | __FUNCTION__ | __LINE__ | __METHOD__ | __PROPERTY__ | __NAMESPACE__ | __TRAIT__ ) \b )
			|   (?<control> \b (?<! \$ ) (?: if | else | elseif | endif | switch | case | break | default | endswitch | match | for | continue | endfor | foreach | as | endforeach | do | while | endwhile | try | catch | finally | throw | return | goto | die | yield from | yield | declare | enddeclare ) \b )
			|   (?<type>    \b (?<! \$ ) (?: bool | int | float | string | array | object | never | void | callable | const | global | static ) \b | \w+: )
			|   (?<value>   \b (?<! \$ ) (?:
			                    (?i: true | false | null )
			                |   (?i: 0x [0-9a-f_]+ | 0b [01_] | 0o? [0-7_]+ )
			                |   [-+]? (?: \d [\d_]* (?: \. [\d_]+ )? | \. [\d_]+ ) (?: [eE] [-+]? [\d_]+ )?
			                ) )
			|   (?<string>  " [\s\S]*? (?<!\\) " | ' [\s\S]*? (?<!\\) ' | <<< '? (?<_id> \w+ ) '? \n [\s\S]+? \g{_id} )
			|   (?<comment> \#.* | //.* | /\* [\s\S]+? \*/ )
			|   (?<call>    \b \w+ (?= \( ) )
			)
		)x
...
function highlight_32_no_nested_groups_no_str_starts_with($code, $language_name) {
	...
	foreach ($matches as $match_index => $match) {
		foreach ($match as $group_name => [$group_text, $group_start_offset]) {
			...
			if ( $group_name[0] == "l" and $group_name[0] == "a" and $group_name[0] == "n" and $group_name[0] == "g" and $group_name[0] == "_" ) {
...
<style>
	...
	span.lang_java { color: hsl(200deg 50% 80%); font-style: italic; }
</style>

<pre><?= highlight_31_nested_groups($code_examples[13][1], $code_examples[13][0]) ?></pre>
<hr>

<?php $start = microtime(true) ?>

<?php foreach($code_examples as [$language, $code]): ?>
<pre><?= highlight_31_nested_groups($code, $language) ?></pre>
<hr>
<?php endforeach ?>

<pre><?= highlight_31_nested_groups(file_get_contents("03_benchmarking.php"), "html") ?></pre>
<hr>
