...
function simple_syntax_highlighting($code, $language_name, $span_attribute_name = "class") {
	$language_definitions = [
		...
		"html" => <<<'EOD'
			...
			|   # HTML start tags
			    (?<variation_c31> < \w+ ) (?<lang_html_attr2> [^>]+? (?: (?: " [^"]*? " | ' [^']*? ' ) [^>]*? )* )? (?<variation_c32> > )
			|   # Doctype
	...
	foreach ($matches as $match_index => $match) {
		foreach ($match as $group_name => [$group_text, $group_start_offset]) {
			...
			if ( $group_name[0] == "l" and $group_name[1] == "a" and $group_name[2] == "n" and $group_name[3] == "g" and $group_name[4] == "_" ) {
				...
				$result .= "<span $span_attribute_name=$group_name>" . simple_syntax_highlighting($group_text, $nested_language) . "</span>";
			} else {
